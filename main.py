...
class SC64MainWidget(QWidget):
    ...
    def list_options(self, command):
        ...
        if options:
            ...
            for opt in options:
                ...
                #btn.clicked.connect(self.run_command)
                btn.clicked.connect(lambda _, option=opt: self.list_parameters(command, option))
                btn.setEnabled(True)
    ...
    def list_parameters(self, command, option):
        ...
        if option[2] > 0 or command == "upload" or command == "download" or command == "64dd":
            ...
            if command == "upload" or command == "download" or command == "64dd":
                ...
                btn.clicked.connect(lambda _: line_edit_of_rom.setText(QFileDialog.getOpenFileName(self, 'Select ROM File', '', 'ROM Files(*.z64 *.n64 *.v64 *.bin)')[0]))
            ...
            if params: # Mostly for RADIO options
                ...
                if option[2] > 0:
                    ...
                    elif type_of_param == Types.PATH:
                        btn = QPushButton("Select Path")
                        btn.setToolTip(option[4])
                        fdOptions = QFileDialog.Option.ShowDirsOnly
                        btn.clicked.connect(lambda _,: 
                                            self.run_command(command, f"{option[1]} {QFileDialog.getSaveFileName(self, "Save File", "", "All Files(*);;Text Files(*.txt)", options = fdOptions)[0]}") if command else "")                
    ...
    def start_worker(self, command, options = "", notify=True):
        ...
        self.worker = ExecWorker(self.exe_path, command, list(options.split()) if options else [])
